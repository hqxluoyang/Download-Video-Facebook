...
public class TaggedFriendActivity extends AppCompatActivity {
    ...
    public void setUpData() {
        ...
        if (token != null) {
            ...
            Toast.makeText(TaggedFriendActivity.this, getResources().getString(R.string.please_login_fb), Toast.LENGTH_SHORT).show();
        }
    ...
    private void loadData() {
        if (cd.isConnectingToInternet()) {
            ...
            fb_user_group_url = "https://graph.facebook.com/v2.2/" + friend_id + "/videos?access_token="
                    + this.token + "&fields=id,picture,created_time,source,updated_time&limit=20";
            addToRequestQueue(new JsonObjectRequest(0, fb_user_group_url, null, new Response.Listener<JSONObject>() {
                    ...
                    if (response != null) {
                        try {
                            ...
                            for (int i = 0; i < jsonArray.length(); i++) {
                                ...
                                Log.e("TAGGED_JSON_OBJECT", jsonObject.toString());
                            ...
                            if (movieArrayList.size() > 0) {
                                myVideoAdapter = new MyVideoAdapter(TaggedFriendActivity.this, movieArrayList);
                ...
                public void onErrorResponse(VolleyError error) {
                    Toast.makeText(TaggedFriendActivity.this, error.getMessage(), Toast.LENGTH_SHORT).show();
                    tvNoVideo.setVisibility(View.VISIBLE);
                    fbProgress.smoothToHide();
                }
            }));
        } else {
            toolsHelper.showAlertDialog(TaggedFriendActivity.this, getResources().getString(R.string.network_connection),
                ...
                public void onResponse(JSONObject response) {
                    if (response != null) {
                        try {
                            ...
                            if (jsonArray.length() == 0) {
                                Toast.makeText(TaggedFriendActivity.this, getResources().getString(R.string.no_more_video), Toast.LENGTH_SHORT).show();
                                rLayout.setVisibility(View.INVISIBLE);
                                fbProgress.smoothToHide();
                                return;
                            }
    ...
    private void getHdLink(String video_id) {
        ...
        addToRequestQueue(new JsonObjectRequest(0, this.urlJsonArry, null, new Response.Listener<JSONObject>() {
            public void onResponse(JSONObject response) {
                try {
                    if (response.has("source")) {
                        TaggedFriendActivity.this.videourl = response.getString("source");
                    } else {
                        TaggedFriendActivity.this.videourl = TaggedFriendActivity.this.videourlmain;
                    }
                    downloadVideoHd();
                } catch (Exception e) {
                }
            ...
            public void onErrorResponse(VolleyError error) {
                fbProgress.setVisibility(View.INVISIBLE);
                Toast.makeText(TaggedFriendActivity.this, getResources().getString(R.string.not_found_link_hd), Toast.LENGTH_SHORT).show();
            }
        }));
    }
...
//                appManagedDownload.main(new String[]{url, toolsHelper.file.getAbsolutePath()});
                Toast.makeText(TaggedFriendActivity.this, getResources().getString(R.string.no_support_download), Toast.LENGTH_SHORT).show();
                ...
                Toast.makeText(TaggedFriendActivity.this, getResources().getString(R.string.video_downloading), Toast.LENGTH_SHORT).show();
            } else {
                Toast.makeText(TaggedFriendActivity.this, getResources().getString(R.string.not_download_video), Toast.LENGTH_SHORT).show();
            }
        }
    }
    ...
    public void showMyDialog() {
        try {
            AlertDialog.Builder builder = new AlertDialog.Builder(TaggedFriendActivity.this);
...
//                            Toast.makeText(getActivity(), "No support play link video youtube and vimeo", Toast.LENGTH_SHORT).show();
                            Intent openVideo = new Intent(TaggedFriendActivity.this, PlayVideoYoutubeActivity.class);
                            ...
                            List<ResolveInfo> intents = TaggedFriendActivity.this.getPackageManager().queryIntentActivities(intent, 0);
                            if (intents != null && intents.size() > 0) {
                                startActivity(Intent.createChooser(intent, getResources().getString(R.string.choose_player)));
                            }
                        } else {
                            Toast.makeText(TaggedFriendActivity.this, getResources().getString(R.string.not_play_video), Toast.LENGTH_SHORT).show();
                        }
                    } else if (position == 3) {
                        dialog.cancel();
                        ToolsHelper.shareLinkVideo(TaggedFriendActivity.this, getResources().getString(R.string.click_video), videourlmain);
                    } else if (position == 4) {
                        dialog.cancel();
                    }
                }
