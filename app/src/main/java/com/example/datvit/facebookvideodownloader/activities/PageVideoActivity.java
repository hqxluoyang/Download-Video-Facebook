...
public class PageVideoActivity extends AppCompatActivity {
    ...
    private void loadData() {
        if (cd.isConnectingToInternet()) {
            ...
            this.page_video_url += this.page_id;
            ...
            addToRequestQueue(new JsonObjectRequest(0, page_video_url, null, new Response.Listener<JSONObject>() {
                    ...
                    if (response != null) {
                        try {
                            ...
                            if (movieArrayList.size() > 0) {
                                myVideoAdapter = new MyVideoAdapter(PageVideoActivity.this, movieArrayList);
                ...
                public void onErrorResponse(VolleyError error) {
                    fbProgress.smoothToHide();
                    Toast.makeText(PageVideoActivity.this, error.getMessage(), Toast.LENGTH_SHORT).show();
                    tvNoVideo.setVisibility(View.VISIBLE);
                }
            }));
        } else {
            toolsHelper.showAlertDialog(PageVideoActivity.this, getResources().getString(R.string.network_connection),
        ...
        if (!this.cd.isConnectingToInternet()) {
            ...
            addToRequestQueue(new JsonObjectRequest(0, nextpage_url, null, new Response.Listener<JSONObject>() {
                public void onResponse(JSONObject response) {
                    ...
                    fbProgress.smoothToHide();
                    Toast.makeText(PageVideoActivity.this, getResources().getString(R.string.no_more_video), Toast.LENGTH_SHORT).show();
                }
            }, new Response.ErrorListener() {
                public void onErrorResponse(VolleyError error) {
                    fbProgress.smoothToHide();
            ...
            builder.setTitle(this.videoname).setAdapter(this.ladapter, new DialogInterface.OnClickListener() {
                public void onClick(DialogInterface dialog, int position) {
                    if (position == 0) {
                        ...
                        dialog.cancel();
                        ToolsHelper.shareLinkVideo(PageVideoActivity.this, getResources().getString(R.string.click_video), videourl);
                    } else if (position == 4) {
                        dialog.cancel();
                    }
                }
        ...
        if (this.cd.isConnectingToInternet()) {
            ...
            if (isSDPresent.booleanValue()) {
                if (url.contains(".mp4")) {
                    ...
                    if (videoid != null) {
                        new_downloadname = PageVideoActivity.this.videoid + ".mp4";
                    } else {
                        new_downloadname = PageVideoActivity.this.videoname + ".mp4";
                    }
                    ...
                    Toast.makeText(PageVideoActivity.this, getResources().getString(R.string.video_downloading), Toast.LENGTH_SHORT).show();
                } else {
                        ...
                        public void onResponse(JSONObject response) {
                            if (response != null) {
                                try {
                                    ...
                                    String mainname = PageVideoActivity.this.getSharedPreferences("pref", 0).getString(str, str2).trim();
                                    File RootFile = new File(Environment.getExternalStorageDirectory() + File.separator + mainname);
                                    if (!RootFile.isDirectory()) {
                                        RootFile.mkdirs();
                                    }
                                    ...
                                    String filename = PageVideoActivity.this.videoname.replaceAll("[-+.^:,]", BuildConfig.VERSION_NAME) + ".mp4";
                                    String sub_path = File.separator + page_name + File.separator + filename;
                                    try {
                                        if (Build.VERSION.SDK_INT >= 11) {
                                            PageVideoActivity.this.lastDownload = PageVideoActivity.this.mgr.enqueue(new DownloadManager.Request(uri).setAllowedNetworkTypes(3)
                                                    .setAllowedOverRoaming(false).setTitle(filename).setDescription(getResources().getString(R.string.downloading))
                                                    .setNotificationVisibility(1).setDestinationInExternalPublicDir(mainname, new_downloadname));
                                        } else {
                                            PageVideoActivity.this.lastDownload = PageVideoActivity.this.mgr.enqueue(new DownloadManager.Request(uri).
                                        ...
                                        Toast.makeText(PageVideoActivity.this, msgnew, Toast.LENGTH_SHORT).show();
                                    }
                                    File f = new File(filename);
                                    PageVideoActivity.this.sendBroadcast(new Intent(Intent.ACTION_MEDIA_SCANNER_SCAN_FILE, Uri.fromFile(f)));
                                    Toast.makeText(PageVideoActivity.this, getResources().getString(R.string.video_downloading), Toast.LENGTH_SHORT).show();
